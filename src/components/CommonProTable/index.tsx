...
import { Button, Space, Popconfirm, Dropdown, Modal, Image, Tooltip } from 'antd';
import { PlusOutlined, EditOutlined, MoreOutlined, EyeOutlined, DeleteOutlined, SaveOutlined } from '@ant-design/icons';
...
interface CommonProTableProps {
    ...
    /** 自定义操作触发器 */
    onAction?: (actionKey: string, action: any, record?: any, isBatch?: boolean, records?: any[]) => void;
    /** 保存操作回调 */
    onSave?: (record: Record<string, any>) => Promise<void>;
    /** 删除操作回调 */
    onDelete?: (record: Record<string, any>) => Promise<void>;
    /** 数据请求函数（当不提供 data 时使用） */
    onRequest?: (params: any) => Promise<{ data: any[], total: number, success: boolean }>;
    /** 额外的 ProTable 属性 */
    tableProps?: any;
}
...
const CommonProTable: React.FC<CommonProTableProps> = ({
    ...
    const generateColumns = useCallback((): ProColumns<Record<string, any>>[] => {
        ...
        Object.entries(modelDesc.fields || {}).forEach(([fieldName, fieldConfig]) => {
            ...
            const column: ProColumns<Record<string, any>> = {
                ...
                hideInTable: false,
                hideInSearch: !(modelDesc.attrs as any)?.list_search?.includes(fieldName),
            ...
            switch (fieldConfig.type) {
                ...
                case 'DateField':
                    ...
                    column.render = (_, record) =>
                        record[fieldName] ? dayjs(record[fieldName]).format('YYYY-MM-DD') : '-';
                    break;
                case 'DatetimeField':
                    column.valueType = 'dateTime';
                    column.render = (_, record) =>
                        record[fieldName] ? dayjs(record[fieldName]).format('YYYY-MM-DD HH:mm:ss') : '-';
                    break;
                case 'TimeField':
                    column.valueType = 'time';
                    column.render = (_, record) =>
                        record[fieldName] ? dayjs(record[fieldName]).format('HH:mm:ss') : '-';
                    break;
                case 'IntegerField':
                case 'FloatField':
                ...
                case 'TextField':
                    if (fieldConfig.choices && fieldConfig.choices.length > 0) {
                        ...
                        column.valueEnum = fieldConfig.choices.reduce((acc: any, [value, label]: [string, string]) => {
                            acc[value] = { text: label };
                            return acc;
                        }, {});
                        column.render = (_, record) => {
                            const choice = fieldConfig.choices?.find(([value]: [string, string]) => value === record[fieldName]);
            ...
            if ((modelDesc.attrs as any)?.list_filter?.includes(fieldName)) {
                if (fieldConfig.choices && fieldConfig.choices.length > 0) {
                    column.filters = fieldConfig.choices.map(([value, label]: [string, string]) => ({
                        text: label,
                        value: value
                    }));
        ...
        if (hasDetailAction || hasEditAction || hasDeleteAction || hasCustomActions) {
            let actionWidth = 80;
                        ...
                        if (isEditing) {
                            ...
                            actions.push(
                                <Button
                                    ...
                                    onClick={async () => {
                                        await onSave?.(record);
                                        setEditableKeys(keys => keys.filter(key => key !== recordKey));
                                    }}
                                    style={{ color: '#52c41a' }}
                                >
                                    Save
                        ...
                        const menuItems = Object.entries(modelDesc.actions || {})
                            ...
                            .map(([actionKey, action]: [string, any]) => {
                                return {
                                    ...
                                    onClick: () => onAction?.(actionKey, action, record, false, [])
                                };
                            });
    ...
    const renderBatchActions = useCallback(() => {
        ...
        const menuItems: any[] = [];
        Object.entries(modelDesc.actions).forEach(([actionKey, action]: [string, any]) => {
            if (action.batch) {
                menuItems.push({
                    key: actionKey,
                    label: action.label || action.name,
